...
export default function Edit(props: EditScheduleProps) {
  ...
  const form = useForm({
    ...
    timezone: page.props.schedule.timezone,
    isDefault: page.props.schedule.isDefault,
  });
                      ...
                      <Flex align="center" gap={12} key={index}>
                        <Select
                          ...
                          data={generateTimeSlots().map((timeSlot) => ({
                            label: format(convertTimeslotToTime(timeSlot), getUserTimeFormat(page.props.auth.user.timeFormat), {locale: ar}),
                            value: timeSlot,
                          }))}
                          onChange={(e) => {
